...
const CONTRIBUTORS_URL =
  "https://raw.githubusercontent.com/Humberd/Stuff-unlocked/master/dist/contributors.json";
!(function () {
  /** @type {!Array} */
  ...
  // Evaluate the script after the page has loaded.
                            ...
                            function init() {
                              expectation.forEach(function (dom, militaryType) {
                                ...
                                const appliedStrength =
                                  militaryData.temporaryStrength || 0;
                                const appliedRank = militaryData.rankNumber;
                                const damageNoHit = calculateNoHitDamage(
                                  appliedStrength,
                                  appliedRank
                                );
                                /** @type {number} */
                        ...
                        if (location.href.includes("economy/myCompanies")) {
                          ...
                          if (
                            (location.href.includes("economy/inventory") ||
                              location.href.includes("main/inventory")) &&
                  ...
                  (citizenProfile.citizen.is_organization
                    ...
                    ? "Q7 hit: " +
                      ...
                      (fighterInfo.military.damagePerHitLegend > 0
                        ? " (TP " +
                          resolve(
                            fighterInfo.military.damagePerHitLegend || ""
                          ) +
                          ")"
                        : "")
                    : "Q0 hit: " +
                      resolve(
                        fighterInfo.aviation.damagePerHitNoWeapon || ""
                      )) +
                  "</brown></div></div>"
                );
              }
    ...
    function claimAllButtonClickHandler(timeToWaitInMs) {
      ...
      if (!claimButtonElements.length) {
        ...
        const closeButtonElement = document.querySelector(
          '#dailyMissionsPopup > a.close.closeButton[title="Close"]'
        );
      ...
      setTimeout(() => {
        claimAllButtonClickHandler(
          Math.floor(300 + Math.random() * (400 - 300 + 1))
        );
      }, timeToWaitInMs);
    }
  }
