    ...
    addEventListener("DOMContentLoaded", function () {
      ...
      document.head.insertAdjacentHTML(
        ...
        "<style>#autoFighter{border:0;position:absolute;width:100vw;height:100vh;z-index:999}#status{position:absolute;width:100vw;height:100vh;z-index:9999;transition:background 1s;background:#000}#status div{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;background:" +
          (!token || token.includes("FUL") ? "#83B70B" : "red") +
          ";color:#fff;padding:5px;font:700 14px Arial;border-radius:1px;text-shadow:0 0 2px #000;box-shadow:0 0 3px inset #000}</style>"
      );
      /** @type {string} */
      document.body.innerHTML =
        '<iframe id="autoFighter" src="/en/military/campaigns" sandbox="allow-same-origin allow-scripts allow-forms"></iframe><div id="status"><div>' +
        (token || "AUTOFIGHTER ON<br>click anywhere to abort") +
        "</div></div>";
      /** @type {string} */
      document.title = "AUTOFIGHTER ON";
      if (0 === navigator.maxTouchPoints) {
        setTimeout(() => {
          return (document.getElementById("status").style.background =
            "rgba(0,0,0,.6)");
        }, 1e3);
      }
                    ...
                    setTimeout(function () {
                      ...
                      function render(m, t) {
                        ...
                        t = t.filter((word, text, fileTitle) => {
                          return (
                            fileTitle.indexOf(word) == text &&
                            "none" != word &&
                            (!params.dailyOrderDone || "DO" != word)
                          );
                        });
                        var d = {};
                          ...
                          if (!args.is_dict && !args.is_lib) {
                            ...
                            if (
                              (value &&
                                $(
                                  args.div,
                                  function (additionalResolves, options) {
                                    /** @type {boolean} */
                                    ...
                                    if (
                                      ...
                                      !options.terrain &&
                                      (id || root) &&
                                      (major || hasDate || capturing) &&
                                      ("both" == highlightLetter ||
                                        ("ground" == highlightLetter &&
                                          !major) ||
                                        ("air" == highlightLetter && major))
                                    ) {
                                      for (let type of t) {
                                        let diH = component[type](
                                          args,
                                          data,
                                          callback,
                                          name,
                                        ...
                                        if (
                                          (diH > -1 &&
                                            d[type][0][
                                              capturing ? "unshift" : "push"
                                            ]([
                                              args.id,
                                              additionalResolves,
                                              "epic" == type
                                                ? id && name == params.country
                                                  ? name
                                                  : root && i == params.country
                                                  ? i
                                                  : newH
                                                : type.includes("TP")
                                                ? params.country
                        ...
                        for (let search_lemma of t) {
                          var args = d[search_lemma][0].concat(
                            d[search_lemma][1]
                          )[0];
                          if (args) {
                            /** @type {boolean} */
                            b = true;
                            localStorage.afKills =
                              "DO" == search_lemma
                                ? 25
                                : "epic" == search_lemma && module
                                ? -1
                                : require("maxKills");
                            value(...args);
                            break;
                      ...
                      var component = {
                        ...
                        DO: function (thing, next, arg, block, id, store) {
                          ...
                          if (body) {
                            for (let node of body) {
                              if (
                                node.battleId == thing.id &&
                                ((next && node.sideCountryId == block) ||
                                  (arg && node.sideCountryId == id) ||
                                  store)
                              ) {
                        ...
                        TP: (pip_offset_base, p, b, i, j, a) => {
                          return (
                            +(
                              (j == params.country && (b || a)) ||
                              (i == params.country && (p || a))
                            ) - 1
                          );
                        },
                      ...
                      if (hasLicense()) {
                        if (
                          (setTimeout(() => {
                            return (location.href =
                              "/" + side + "/military/campaigns");
                          }, 60 * (ms ? 15 : 5) * 1e3),
                          (top.lastCheck = Date.now()),
                          chans)
                        ) {
                          var unhandledDepCount =
                            globalNS.userInfo.wellness + food_remaining;
                          ...
                          if (
                            require("collectWcRewards") &&
                            localStorage.afKills
                          ) {
                            callback(
                              "/" + side + "/main/weekly-challenge-collect-all",
                              {
                                _token: csrfToken,
                              }
                            );
                          ...
                          if (
                            params.dailyOrderDone &&
                            !params.hasDailyOrderReward
                          ) {
                          ...
                          /** @type {number} */
                          var q = parseInt(
                            document.getElementById("live_time").textContent
                          );
                          if (
                            unhandledDepCount > 50 &&
                            Math.abs(
                              (localStorage.workTrainLastAttempt || -9) - q
                            ) > 1
                          ) {
                            ...
                            if (!params.dailyTasksDone && require("train")) {
                              test(
                                "/" + side + "/main/training-grounds-json",
                                function (that) {
                                  var data = {
                                    _token: csrfToken,
                                  };
                            ...
                            if (html || txt) {
                              test("/" + side + "/main/job-data", function (s) {
                                if (html && !s.alreadyWorked) {
                                  ...
                                  if (
                                    txt &&
                                    1e3 * s.overTime.nextOverTime - Date.now() <
                                      0 &&
                                    s.overTime.points > 23
                                  ) {
                                    callback(
                                      "/" + side + "/economy/workOvertime",
                                      {
                          ...
                          if (
                            require("buyMMgold") &&
                            now != localStorage.boughtGoldDay
                          ) {
                            callback(
                              ...
                              function (relativeResource) {
                                ...
                                var createdCollection =
                                  relativeResource.buy_mode.split("purchase_");
                                /** @type {!Array} */
                                var stack = [];
                                for (
                                  let j = 1;
                                  j < createdCollection.length;
                                  j++
                                ) {
                                  let value = createdCollection[j];
                                  if (
                                    parseInt(value.split("data-max='")[1]) >= 10
                                  ) {
                                ...
                                !(function e() {
                                  callback(
                                    "/" + side + "/economy/exchange/purchase/",
                                    {
                                      _token: csrfToken,
                                      offerId: stack.shift(),
                                      amount: 10,
                                    ...
                                    function (importError) {
                                      if (
                                        /does not exist|more than the offered/.test(
                                          importError.message
                                        ) &&
                                        stack.length
                                      ) {
                            ...
                            if (
                              ...
                              handlebars &&
                              recentFiles.length &&
                              +localStorage.getItem("wamAttempt") <
                                movies.length + 2
                            ) {
                              ...
                              if (
                                ...
                                Math.min(
                                  10 * tmp[1].length,
                                  2 * reset_health_to_recover - 10
                                )
                              ) {
                                /** @type {number} */
                                var total_seconds = (localStorage.wamAttempt =
                                  +localStorage.getItem("wamAttempt") + 1);
                                var infos = tmp[1];
                                let r = {
                              ...
                              if (
                                Handlebars &&
                                errors.length &&
                                now != localStorage.assignedEmployeesDay
                              ) {
                          ...
                          if (ms) {
                            if (SERVER_DATA.webDeployEnabled) {
                              callback("/" + side + "/main/profile-update", {
                                action: "options",
                                params:
                                  '{"optionName":"enable_web_deploy","optionValue":"' +
                                  (flightMode ? "on" : "off") +
                                  '"}',
                                _token: csrfToken,
                              });
                              ...
                              if (
                                (value < 0
                                  ? ((document.getElementById(
                                    ...
                                    module &&
                                      document.getElementById("allin").click())
                                  : globalNS.userInfo.wellness >
                                      reset_health_to_recover &&
                                    (document.getElementById("allin").click(),
                                    setInterval(function () {
                                ...
                                (document.getElementById("kills").value =
                                  value < 0 ? require("maxKills") : value),
                                hasLicense())
                              ) {
                                /** @type {(Element|null)} */
                                var t =
                                  document.getElementById("AutoBotSwitch");
                                t.click();
                                setInterval(function () {
